...
`PRD 만들자` 또는 `리플랜으로 검토해줘`라는 한 문장에서 시작해 빌드 위험, 문제와 가설, 인터뷰, 5개 PRD 문서, 비판 검수, 예외·완료 조건, 측정 설계, 승인 후 실행 계획까지 연결하는 Codex 플러그인 번들입니다.
...
- `prd-replan` — 고위험 계획을 최소 두 가지 대안, ADR-lite, 순차 검수와 승인 대기로 검토합니다.
...
- `리플랜으로 검토해줘` → 변경 없이 선택지·리스크·검증을 담은 `pending approval` 결정 기록
