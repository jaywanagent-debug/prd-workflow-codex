...
This file is modified for the `prd-workflow-codex` bundle. When feature flows are not specified, use the bundled `show-me-the-prd` before defining instrumentation.
...
- The feature itself is not yet specified (no flows to instrument) -> use bundled `show-me-the-prd` first
