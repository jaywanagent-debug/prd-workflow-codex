...
-> `define-hypothesis` to frame "SSO unblocks >= $X enterprise ARR this quarter" as a testable claim with a metric and a date. Once the spike gives a real cost, compare it against activation work with an installed prioritization method or a constrained shortlist. If a signed contingency comes back, re-run this review; a confirmed L4 flips the verdict to Build small and routes to bundled `define-problem-statement` -> `show-me-the-prd`.
