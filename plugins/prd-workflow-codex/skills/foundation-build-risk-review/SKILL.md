...
This file is modified for the `prd-workflow-codex` bundle. A **Build small** verdict routes to bundled `define-problem-statement`, `define-hypothesis`, and `show-me-the-prd`. A **Validate first** verdict routes to bundled `define-hypothesis`.

Do not bypass the bundled `goaljaby` for Codex `/goal` handoff. Skills not included in this bundle may be suggested as optional follow-up, but the review must always provide a usable no-code next step without requiring them.
...
| Verdict | Routes to |
|---|---|
| Build small | bundled `define-problem-statement`, `define-hypothesis`, then `show-me-the-prd` |
| Validate first | bundled `define-hypothesis`, then run its no-code validation approach |
| Pivot first | stop and document the wedge, user, and value proposition that must be reframed |
| Don't build yet | stop and state what evidence would justify reopening the decision |
| Several competing requests | return a constrained shortlist; suggest a prioritization skill only if installed |

The upstream map is retained in `references/routing-map.md` for attribution and optional follow-up. The bundled routing table above takes precedence.
