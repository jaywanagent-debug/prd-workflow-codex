...
name: pm-critic
description: |
  Use proactively after any PM-artifact-producing skill completes (show-me-the-prd,
...
## Bundled Install Note

This redistributed file is modified for the `prd-workflow-codex` bundle, which does not include the full upstream `docs/` tree. Use the active bundled skill's `SKILL.md` and `references/TEMPLATE.md`. For PRDs, use bundled `show-me-the-prd` as the authoring standard and cross-reference the bundled edge-case, acceptance-criteria, hypothesis, and instrumentation skills.
...
- **Meeting recaps / agendas / briefs / syntheses / stakeholder updates:** read the producing skill's `SKILL.md` for its output contract; if a family contract is not bundled, infer shared family rules from the active meeting/foundation skill files
- **Personas:** read `skills/foundation-persona/SKILL.md` for evidence calibration and assumption-flagging requirements
- **PRDs:** read `skills/show-me-the-prd/SKILL.md`; cross-reference `skills/deliver-edge-cases/SKILL.md`, `skills/deliver-acceptance-criteria/SKILL.md`, `skills/define-hypothesis/SKILL.md`, and `skills/measure-instrumentation-spec/SKILL.md`
...
- **Foundation Sprint outputs:** read the relevant active foundation skill's `SKILL.md`; if no family contract is bundled, infer the contract from sibling foundation skills
- **Design Sprint outputs:** read the relevant active design sprint skill's `SKILL.md`; if no family contract is bundled, infer the contract from sibling design sprint skills
...
Produce a markdown findings report following this structure (local full format available in `skills/utility-pm-critic/references/TEMPLATE.md`):
...
When reviewing PM artifacts (PRDs, OKR sets, personas), produce a narrow, decision-ready findings artifact. Use the severity grammar above and include concrete fixes for every finding.
...
- Local output template: `skills/utility-pm-critic/references/TEMPLATE.md`
- Local worked example: `skills/utility-pm-critic/references/EXAMPLE.md`
